...
Our Paper is : Kink Finder: A Better Way to Place Kinks. Details of the algorithm can be found in the supplementary material
Any problems, queries, or suggestions, please contact henry.wilman@dtc.ox.ac.uk. 


Kink Finder locates kinks in helices. Based on the local structure of the helix kink, it identifies a residue on the inside of the kink as the kink residue. An angle for each residue is calculated by fitting the coordinates of the 4 backbone atoms of that residue and each of the 5 residues before it (i.e. towards the n-terminus of the protein) to a cylinder, and the backbone atoms of the six residues after it, and calculating the angle between the two cylinder axes. Kinks are identified starting with the residue with the largest angle, subsequently the residues are examined in decreasing order of angle, with a residue being identified as a kink if it is at least 5 residues away from an existing kink, and there is a residue between it and all existing kinks that has an angle less than 10. For each kink, the kink residue is selected by looking at the residue initially identified as the kink, the residue before, and the two after, and choosing the residue most 'inside' the kink is as the kink angle, based on the 'wobble' angle 
...
Mac: 
----
Download the file Kink_Finder_mac.tar.gz

tar -zxvf Kink_Finder.tar.gz

A pre-complied binary is supplied, but it *may* not work on your system.
Further instructions on compiling the binary can be found in the troubleshooting section below.


Windows:
--------
Download the file Kink_Finder_windows.zip
...
Note:
-----
A pre-compiled C binary (or executable for windows) is supplied, which performs the cylinder fits. For linux and windows systems, this works on a wide range of systems, however it may not work on yours. The source code is supplied, so if it does not work (see troubleshooting section), you may need to compile it on your system, for which instructions are supplied.
...
helices.csv: Comma separated file, with each line detailing information for an analysed helix: the chain code, the first residue in the helix, the last residue in the helix, the number of the kink residue (which is on the inside of the kink), and the maximum angle in the helix.

kinks.csv: Comma separated file, with each line showing a kink over 10 degrees. The 13 residues around the kink residue are shown (6 before, the kink residue itself, and the 6 after).  There are fields for the chain code, the start and end residue number of the helix, the number of the kink residue, the start and end residue number of the 13 residue segment, the kink angle, the sequence of the 13 residues segment around the kink. The last 4 fields give information about the cylinder fits used to calculate the kink angle. These are the rmsd from the cylinder surface, and the radius of the cylinder fitted to the n-terminal and c-terminal side of the kink.

angles.csv: Comma separated file. Each row starts with the pdb code, chain identifier, and index of the first helix residue concatednated together. Each subsequent entry is the calculated angle for each residue of the helix. The first 5 and last 6 are set to 0, as no angle is calcuated for these residues.


More advanced usage:
--------------------

To use your own helix definitions, split the pdb file into its constituent chains, and input the pdb numbers of the first and last residues in each helix, like this:
...
The pre-compiled binary is not working on your system. Ensure you are using the correct version of Kink Finder (windows, mac, linux). If the problem persists, the source code is provided in the folder ./cylinder_source_code/ 

To complile (linux):
cd to the kink finder directory, and then type
make install

alternatively, without the make file:
